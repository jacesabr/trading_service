"""rlab.harness — the leak-safe R&D engine (leak test / walk-forward / grid).

The cardinal rule (NO LOOKAHEAD) is enforced HERE, not by signal authors. Every
prediction is made as_of bar t and scored on bar t+1's independent close. Two
evaluation paths, both leak-free, both predicting the next independent bar:

  * rule-vectorized — when a manifest's signal is a feature-threshold rule set
    (e.g. meanrev via rules.json), evaluate it over features(df, lag=False) in
    one vectorized pass. feats[t] is the indicator AT bar t; the rule at t
    predicts the close of t+1 — identical semantics to the live path and to
    indicator_battery's lagged construction, just indexed off t instead of t-1.
  * windowed-callable — universal fallback for arbitrary agent-authored signal
    fns: slide a bounded trailing window ending at t, call the fn, predict t+1.
    Bounded window (default 300) keeps it O(n) and reproduces the full-history
    indicator values (all indicators here are local, <=144-bar).

binary/directional kinds are fully implemented. bracket (SL/TP) resolution needs
1m data + zone construction and lands with the P2 adapters.
"""
import importlib
import math
import os
...
DEFAULT_WINDOW = 300
MIN_N = 50            # too-few guard (matches gap_next5m)
...
def _load_chart(symbol, tf):
    """P1 binance loader: read the local {SYMBOL}_{tf}.csv (data.py dumps).
    P2 routes this through adapters/data/<adapter>.py."""
    path = os.path.join(REPO, f"{symbol}_{tf}.csv")
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"{symbol}_{tf}.csv missing — fetch with: python data.py {symbol} "
            f"{tf} 1m <start_ym> <end_ym>")
...
def _rules_for(manifest):
    """Return the feature-threshold rule list if this is a rule strategy, else
    None. Supports inline signal.rules or a rules_file (e.g. rules.json)."""
    sig = manifest["signal"]
    if "rules" in sig:
        return sig["rules"]
...
def _predict_callable(df, fn, params, window, lo=0, hi=None):
    """Predictions for the scored range only — callers pass [lo:hi) so a TEST-
    or VAL-only evaluation doesn't pay to scan the whole history per combo."""
    n = len(df)
...
def _predictions(df, manifest, params=None, lo=0, hi=None):
    """Dispatch to the fast rule path (full, vectorized) or the universal
    callable path (restricted to [lo:hi) for speed)."""
    if manifest.get("kind") == "bracket":
        raise HarnessPending(
            "bracket (SL/TP) resolution needs 1m data + zone construction — "
            "lands with the P2 adapters. binary/directional work now.")
...
def _score(df, pred, lo=0, hi=None):
    """Score predictions on bars [lo:hi). pred[t] predicts sign(c[t+1]-o[t+1]).
...
def leaktest(manifest, **kw):
    """Leak-free held-out hit on the TEST slice, pooled across the strategy's
    symbols. PASS = enough samples AND TEST z >= 2 (a real, non-50% tilt)."""
    tf = _tf(manifest)
    per = {}
    ...
    for sym in _symbols(manifest):
        try:
            ...
            continue
        _, i2 = _splits(len(df))
        pred = _predictions(df, manifest, lo=i2)  # predict TEST slice only
        s = _score(df, pred, lo=i2)              # TEST only
        per[sym] = s
        if s["n"]:
    ...
    return dict(passed=bool(z >= 2.0), n=pooled_n, hit=hit, z=z,
                per_symbol=per,
...
def walkforward(manifest, n_windows=5, embargo=12, **kw):
    """Rolling out-of-sample windows with an embargo gap (purge) before each
    test block. Robust = positive (z>0) in >=ceil(0.8*k) windows on EVERY
    symbol — the STRATEGY.md bar (>=4/5, both symbols)."""
...
def gridsearch(manifest, **kw):
    ...
    the chance-max-z to beat so a swept winner isn't mistaken for an edge. NOTE:
    the signal fn must accept a params dict for the sweep to change behavior."""
    ...
    for params in combos:
        ...
        for sym in _symbols(manifest):
            ...
            pred = _predictions(df, manifest, params=params,
                                lo=max(0, i1 - win), hi=i2)  # VAL slice only
            s = _score(df, pred, i1, i2)         # VALIDATION
            if s["n"]:
                pooled_n += s["n"]; pooled_hit_w += s["hit"] * s["n"]
        if pooled_n >= MIN_N:
            hit = pooled_hit_w / pooled_n
